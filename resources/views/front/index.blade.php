...
<body class="font-[Poppins] pb-[72px]">
    ...
    <section id="Featured" class="mt-[30px]">
        <div class="main-carousel w-full">
            <!-- Carousel News -->
            @forelse($featuredArticles as $featuredArticle)
    ...
    <section id="Up-to-date" class="max-w-[1130px] mx-auto flex flex-col gap-[30px] mt-[70px]">
        ...
        <div class="grid grid-cols-3 gap-[30px]">
            ...
            @forelse($articles as $article)
                <a href="{{ route('front.detail', $article->slug) }}" class="card-news">
                    <div
    ...
    <section id="Latest-entertainment" class="max-w-[1130px] mx-auto flex flex-col gap-[30px] mt-[70px]">
        ...
        <div class="flex justify-between items-center h-fit"> 
            <div class="featured-news-card relative w-full h-[424px] flex flex-1 rounded-[20px] overflow-hidden">
                <img src="{{ Storage::url($featuredEntertainmentArticles->thumbnail) }}"
                    class="thumbnail absolute w-full h-full object-cover" alt="icon" />
                <div class="w-full h-full bg-gradient-to-b from-[rgba(0,0,0,0)] to-[rgba(0,0,0,0.9)] absolute z-10">
                </div>
                <div class="card-detail w-full flex items-end p-[30px] relative z-20">
                    <div class="flex flex-col gap-[10px]">
                        <p class="text-white">Featured</p>
                        <a href="{{ route('front.detail', $featuredEntertainmentArticles->slug) }}"
                            class="font-bold text-[30px] leading-[36px] text-white hover:underline transition-all duration-300">{{ substr($featuredEntertainmentArticles->name, 0, 50) }}{{ strlen($featuredEntertainmentArticles->name) > 50 ? '...' : '' }}</a>
                        <p class="text-white">{{ $featuredEntertainmentArticles->created_at->format('d M, Y') }}</p>
                    </div>
                </div>
            </div> 
            <div class="h-[424px] w-fit px-5 overflow-y-scroll overflow-x-hidden relative custom-scrollbar">
                <div class="w-[455px] flex flex-col gap-5 shrink-0">
                    @forelse($entertainmentArticles as $entertainmentarticle)
                    <a href="{{ route('front.detail', $entertainmentarticle->slug) }}" class="card py-[2px]">
                        <div
                            ...
                            <div class="w-[130px] h-[100px] flex shrink-0 rounded-[20px] overflow-hidden">
                                <img src="{{ Storage::url($entertainmentarticle->thumbnail) }}" class="object-cover w-full h-full"
                                    alt="thumbnail" />
                            </div>
                            <div class="flex flex-col justify-center-center gap-[6px]">
                                <h3 class="font-bold text-lg leading-[27px]">{{ substr($entertainmentarticle->name, 0, 50) }}{{ strlen($entertainmentarticle->name) > 50 ? '...' : '' }}</h3>
                                <p class="text-sm leading-[21px] text-[#A3A6AE]">{{ $entertainmentarticle->created_at->format('d M, Y') }}</p>
                            </div>
                        </div>
                    </a>
                    @empty
    ...
    <section id="Latest-business" class="max-w-[1130px] mx-auto flex flex-col gap-[30px] mt-[70px]">
        ...
        <div class="flex justify-between items-center h-fit"> 
            <div class="featured-news-card relative w-full h-[424px] flex flex-1 rounded-[20px] overflow-hidden">
                <img src="{{ Storage::url($featuredBusinessArticles->thumbnail) }}"
                    class="thumbnail absolute w-full h-full object-cover" alt="icon" />
                <div class="w-full h-full bg-gradient-to-b from-[rgba(0,0,0,0)] to-[rgba(0,0,0,0.9)] absolute z-10">
                </div>
                <div class="card-detail w-full flex items-end p-[30px] relative z-20">
                    <div class="flex flex-col gap-[10px]">
                        <p class="text-white">Featured</p>
                        <a href="{{ route('front.detail', $featuredBusinessArticles->slug) }}"
                            class="font-bold text-[30px] leading-[36px] text-white hover:underline transition-all duration-300">{{ substr($featuredBusinessArticles->name, 0, 50) }}{{ strlen($featuredBusinessArticles->name) > 50 ? '...' : '' }}</a>
                        <p class="text-white">{{ $featuredBusinessArticles->created_at->format('d M, Y') }}</p>
                    </div>
                </div>
            </div> 
            <div class="h-[424px] w-fit px-5 overflow-y-scroll overflow-x-hidden relative custom-scrollbar">
                <div class="w-[455px] flex flex-col gap-5 shrink-0">
                    @forelse($businessArticles as $businessarticle)
                    <a href="{{ route('front.detail', $businessarticle->slug) }}" class="card py-[2px]">
                        <div
                            class="rounded-[20px] border border-[#EEF0F7] p-[14px] flex items-center gap-4 hover:ring-2 hover:ring-[#FF6B18] transition-all duration-300">
                            <div class="w-[130px] h-[100px] flex shrink-0 rounded-[20px] overflow-hidden">
                                <img src="{{ Storage::url($businessarticle->thumbnail) }}" class="object-cover w-full h-full"
                                    alt="thumbnail" />
                            </div>
                            <div class="flex flex-col justify-center-center gap-[6px]">
                                <h3 class="font-bold text-lg leading-[27px]">{{ substr($businessarticle->name, 0, 50) }}{{ strlen($businessarticle->name) > 50 ? '...' : '' }}</h3>
                                <p class="text-sm leading-[21px] text-[#A3A6AE]">{{ $businessarticle->created_at->format('d M, Y') }}</p>
                            </div>
                        </div>
                    </a> 
                    @empty
    ...
    <section id="Latest-automotive" class="max-w-[1130px] mx-auto flex flex-col gap-[30px] mt-[70px]">
        ...
        <div class="flex justify-between items-center h-fit"> 
            <div class="featured-news-card relative w-full h-[424px] flex flex-1 rounded-[20px] overflow-hidden">
                <img src="{{ Storage::url($featuredAutomotiveArticles->thumbnail) }}"
                    class="thumbnail absolute w-full h-full object-cover" alt="icon" />
                <div class="w-full h-full bg-gradient-to-b from-[rgba(0,0,0,0)] to-[rgba(0,0,0,0.9)] absolute z-10">
                </div>
                <div class="card-detail w-full flex items-end p-[30px] relative z-20">
                    <div class="flex flex-col gap-[10px]">
                        <p class="text-white">Featured</p>
                        <a href="{{ route('front.detail', $featuredAutomotiveArticles->slug) }}"
                            class="font-bold text-[30px] leading-[36px] text-white hover:underline transition-all duration-300">{{ substr($featuredAutomotiveArticles->name, 0, 50) }}{{ strlen($featuredAutomotiveArticles->name) > 50 ? '...' : '' }}</a>
                        <p class="text-white">{{ $featuredAutomotiveArticles->created_at->format('d M, Y') }}</p>
                    </div>
                </div>
            </div> 
            <div class="h-[424px] w-fit px-5 overflow-y-scroll overflow-x-hidden relative custom-scrollbar">
                <div class="w-[455px] flex flex-col gap-5 shrink-0">
                    @forelse($automotiveArticles as $automotivearticle)
                    <a href="{{ route('front.detail', $automotivearticle->slug) }}" class="card py-[2px]">
                        <div
                            class="rounded-[20px] border border-[#EEF0F7] p-[14px] flex items-center gap-4 hover:ring-2 hover:ring-[#FF6B18] transition-all duration-300">
                            <div class="w-[130px] h-[100px] flex shrink-0 rounded-[20px] overflow-hidden">
                                <img src="{{ Storage::url($automotivearticle->thumbnail) }}" class="object-cover w-full h-full"
                                    alt="thumbnail" />
                            </div>
                            <div class="flex flex-col justify-center-center gap-[6px]">
                                <h3 class="font-bold text-lg leading-[27px]">{{ substr($automotivearticle->name, 0, 50) }}{{ strlen($automotivearticle->name) > 50 ? '...' : '' }}</h3>
                                <p class="text-sm leading-[21px] text-[#A3A6AE]">{{ $automotivearticle->created_at->format('d M, Y') }}</p>
                            </div>
                        </div>
                    </a> 
                    @empty
    ...
    <footer id="footer" class="max-w-[1130px] mx-auto flex justify-between items-center mt-[70px]">
        <div class="flex flex-col gap-[30px]">
            ...
            <p class="font-medium text-sm leading-[21px] text-[#A3A6AE]">
                © 2022 News App. All rights reserved
            </p>
        </div>
        {{-- <div class="flex items-center gap-3">
            <a href=""
